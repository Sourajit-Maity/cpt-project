<x-admin.table>
    ...
    <x-slot name="PageButton">
        <div class="subadmin_btn">
            <a href="{{route('shift-time.create')}}" class="btn btn-brand btn-elevate btn-icon-sm">+ Add New Shift Time</a>
        </div>
    </x-slot>
    ...
    <x-slot name="thead">
        <tr role="row">
            <th tabindex="0" aria-controls="kt_table_1" rowspan="1" colspan="1" style="width: 22%;"
                aria-sort="ascending" aria-label="Agent: activate to sort column descending">Type Name <i
                    class="fa fa-fw fa-sort pull-right" style="cursor: pointer;" wire:click="sortBy('shift_name')"></i>
            </th>
            <th tabindex="0" aria-controls="kt_table_1" rowspan="1" colspan="1" style="width: 22%;"
                aria-sort="ascending" aria-label="Agent: activate to sort column descending">Shift Time  <i
                    class="fa fa-fw fa-sort pull-right" style="cursor: pointer;" wire:click="sortBy('shift_time')"></i>
            </th>
        ...
        <tr class="filter">
            ...
            <th>
                <x-admin.input type="search" wire:model.defer="searchTime" placeholder="" autocomplete="off"
                    class="form-control-sm form-filter" />
            </th>
    ...
    <x-slot name="tbody">
        @forelse($types as $data)
            ...
            <td>{{ $data->shift_name }}</td>
                ...
                <x-admin.td-action>
                    <a class="dropdown-item" href="{{ route('shift-time.edit', ['shift_time' => $data->id]) }}"><i class="la la-edit"></i> Edit</a>
                    <button href="#" class="dropdown-item" wire:click="deleteAttempt({{ $data->id }})">
